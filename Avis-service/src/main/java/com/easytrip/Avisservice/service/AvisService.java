...
@Override
public Avis updateAvis(Long id, Avis updatedAvis) {
    Avis existingAvis = getAvisById(id);  // Récupérer l'avis existant
    existingAvis.setNote(updatedAvis.getNote());  // Mettre à jour la note
    existingAvis.setCommentaire(updatedAvis.getCommentaire());  // Mettre à jour le commentaire
    existingAvis.setApprouve(updatedAvis.isApprouve());  // Mettre à jour l'état d'approbation
    return avisRepository.save(existingAvis);  // Sauvegarder et renvoyer l'avis mis à jour
}
    ...
    public List<Avis> searchAvis(String keyword, Long voyageId, Boolean approuve) {
        // Implémenter la logique de recherche en utilisant les paramètres
        // Par exemple, un critère de recherche par mot-clé dans le commentaire et le voyageId
        // Et un critère pour filtrer par approuvé (true/false)
        return avisRepository.findAll().stream()
                .filter(avis -> (keyword == null || avis.getCommentaire().contains(keyword)) &&
                        (voyageId == null || avis.getVoyageId().equals(voyageId)) &&
                        (approuve == null || avis.isApprouve() == approuve))
                .collect(Collectors.toList());
    }
